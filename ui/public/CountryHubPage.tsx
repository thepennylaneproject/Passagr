...
export const CountryHubPage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto p-4 md:p-10">
            ...
            <h2 className="text-2xl font-bold mb-4 text-surface-500">Safety & Legal Climate</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                ...
                <div className="p-4 border border-gray-200 rounded-lg shadow-md bg-var(--color-ivory) md:col-span-1">
                    <h3 className="text-xl font-bold mb-1 text-gray-800">Abortion Access</h3>
                    <p className="text-lg font-semibold text-green-600">{country.abortion_access_status}</p>
            ...
            <h2 className="text-2xl font-bold mb-4 text-surface-500">Living & Financial Overview</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            ...
            <section>
                ...
                <div className="space-y-6">
                    {visaPaths.map((path) => (
                        <a key={path.id} href={`/visa-paths/${path.id}`} className="block border rounded-lg p-6 hover:bg-accent-50 transition duration-150 ease-in-out">
                            <div className="flex justify-between items-start">
                                <div>
                                    <span className="text-xs font-semibold uppercase text-surface-600 tracking-wider">{path.type.replace('_', ' ')}</span>
                                    <h3 className="text-2xl font-bold text-gray-900 mt-1">{path.name}</h3>
                                </div>
                                <div className="text-right">
                                    <p className="text-lg font-bold text-surface-600">~{path.estimated_first_year_cost.toLocaleString()} {path.currency}</p>
                            ...
                            <div className="mt-4 flex space-x-6 text-sm">
                                ...
                                <span className="text-surface-600">{path.to_pr_citizenship_timeline}</span>
                            </div>
                        </a>
                    ))}
                </div>
