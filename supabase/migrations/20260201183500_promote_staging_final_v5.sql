...
-- Fixes JSON type mismatches for special_regimes and years_to_citizenship.
  ...
  for v_r in select * from staging_country_research loop
    ...
    for v_pathway in select * from jsonb_array_elements(v_r.payload->'pathways') loop
      ...
      values (
        ...
        coalesce(v_pathway->'eligibility', '[]'::jsonb),
        (v_r.payload->'country'->>'last_verified_at')::timestamptz
      )
    ...
    values (
      ...
      'Not specific',
      case 
        when jsonb_typeof(v_r.payload->'taxation'->'special_regimes') = 'array' 
        then coalesce(v_r.payload->'taxation'->'special_regimes'->0->>'notes', 'None')
        else coalesce(v_r.payload->'taxation'->>'special_regimes', 'None')
      end
    )
    on conflict (country_id) do update set
    ...
    values (
      ...
      case 
        when jsonb_typeof(v_r.payload->'language_family_citizenship'->'years_to_citizenship') = 'object'
        then coalesce((v_r.payload->'language_family_citizenship'->'years_to_citizenship'->>'min')::text, '5')
        else coalesce((v_r.payload->'language_family_citizenship'->>'years_to_citizenship')::text, '5')
      end,
